...
function BookingModal({ doctor, user, onClose, onBooked }) {
  ...
  // 🔀 DECIDE: generate next 5 available days starting from tomorrow
  const days = Array.from({ length: 5 }, (_, i) => {
    const date = new Date();
    date.setDate(date.getDate() + i + 1);
    return {
      label: date.toLocaleDateString('fr-DZ', { weekday: 'short', day: 'numeric', month: 'short' }),
      value: date.toISOString().split('T')[0], // format: 2026-03-15
    };
  });

  // 🔀 DECIDE: available time slots
  const slots = ['08:00', '09:00', '10:00', '11:00', '14:00', '15:00', '16:00', '17:00'];

  // 💾 SAVE: send appointment to database
  const confirmBooking = async () => {
    // 🔀 DECIDE: make sure patient picked a day AND a slot
    if (!selectedDay || !selectedSlot) return;

    setLoading(true);

    // 💾 SAVE: insert a new row in the appointments table
    const { error } = await supabase.from('appointments').insert({
      user_id: user.id,        // who is booking
      doctor_id: doctor.id,    // which doctor
      appointment_date: selectedDay,   // which day
      appointment_time: selectedSlot,  // which time
      status: 'confirmed',     // default status
      type: 'Consultation',    // default type
      price: doctor.price,     // doctor's price
    ...
    if (error) {
      ...
      setConfirmed(true); // 🔀 DECIDE: show success screen
      onBooked();
    }
    setLoading(false);
  };
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(4px)' }}>
      <div style={{ ...S.card, width: 480, position: 'relative', maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        {/* 👁️ SHOW: success screen after booking */}
        {confirmed ? (
          <div style={{ textAlign: 'center', padding: '20px 0' }}>
            ...
            <div style={{ background: '#f0f6ff', borderRadius: 14, padding: 20, textAlign: 'left', marginBottom: 24 }}>
              {[
                ['👨‍⚕️ Médecin', doctor.full_name],
                ['🏥 Spécialité', doctor.specialty],
                ['📅 Date', selectedDay],
                ['⏰ Heure', selectedSlot],
                ['💰 Prix', `${doctor.price} DA`],
              ].map(([label, value]) => (
          ...
          <>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 24, flexWrap: 'wrap' }}>
              {days.map(day => (
                <button key={day.value} onClick={() => setSelectedDay(day.value)}
                  style={{ padding: '10px 14px', borderRadius: 10, border: '1.5px solid', cursor: 'pointer', fontFamily: 'inherit', fontSize: 13, fontWeight: 600, transition: 'all 0.2s',
                    borderColor: selectedDay === day.value ? '#0057b8' : '#e0e8f5',
                    background: selectedDay === day.value ? '#0057b8' : 'white',
                    color: selectedDay === day.value ? 'white' : '#555' }}>
                  {day.label}
                </button>
              ))}
            </div>

            {/* 👁️ SHOW: time slots — only appears after picking a day */}
            {selectedDay && (
              <>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, marginBottom: 24 }}>
                  {slots.map(slot => (
                    <button key={slot} onClick={() => setSelectedSlot(slot)}
                      style={{ padding: '10px', borderRadius: 10, border: '1.5px solid', cursor: 'pointer', fontFamily: 'inherit', fontSize: 14, fontWeight: 700, transition: 'all 0.2s',
                        borderColor: selectedSlot === slot ? '#0057b8' : '#e0e8f5',
                        background: selectedSlot === slot ? '#0057b8' : 'white',
            ...
            {/* 👁️ SHOW: confirm button — only active when both day and slot are picked */}
...
function LoginScreen({ onClose, onSwitchToRegister, onLogin }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(4px)' }}>
      <div style={{ ...S.card, width: 420, position: 'relative' }}>
        {/* Close */}
        <button onClick={onClose} style={{ position: 'absolute', top: 16, right: 16, background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: '#999' }}>✕</button>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <input value={email} onChange={e => setEmail(e.target.value)}
            placeholder="votre@email.com" type="email" style={S.input} />
        </div>
        <div style={{ marginBottom: 20 }}>
          <label style={S.label}>MOT DE PASSE</label>
          <input value={password} onChange={e => setPassword(e.target.value)}
            placeholder="••••••••" type="password" style={S.input}
            onKeyDown={e => e.key === 'Enter' && handleLogin()} />
        </div>

        <button onClick={handleLogin} style={S.btnPrimary} disabled={loading}>
          {loading ? '⏳ Connexion...' : '🔐 Se connecter'}
        </button>

        <div style={{ textAlign: 'center', marginTop: 20, fontSize: 14, color: '#888' }}>
          Pas encore de compte ?{' '}
          <span onClick={onSwitchToRegister} style={{ color: '#0057b8', fontWeight: 700, cursor: 'pointer' }}>
            S'inscrire
          </span>
        </div>
      </div>
...
function RegisterScreen({ onClose, onSwitchToLogin, onLogin }) {
  ...
  const handleRegister = async () => {
    ...
    // Save extra info in users table
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(4px)' }}>
      <div style={{ ...S.card, width: 420, position: 'relative', maxHeight: '90vh', overflowY: 'auto' }}>
        <button onClick={onClose} style={{ position: 'absolute', top: 16, right: 16, background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: '#999' }}>✕</button>
        ...
        <button onClick={handleRegister} style={{ ...S.btnPrimary, marginTop: 6 }} disabled={loading}>
...
// 💾 GET: check if user is already logged in when app loads
useEffect(() => {
  // Get current session
...
const [rdvLoading, setRdvLoading] = useState(false);
  ...
  const [screen, setScreen] = useState(null); // 'login' | 'register' | null
...
const fetchMyAppointments = async () => {
  ...
  // 💾 GET: fetch all appointments for this user
  // and join with doctors table to get doctor name
...
if (error) console.log('ERROR:', error);
  ...
  />
      ...
      {/* HEADER */}
      <div style={{ background: 'linear-gradient(135deg, #0057b8, #0096c7)', padding: '0 24px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', height: 64 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }} onClick={() => { setSearched(false); setDoctors([]); }}>
          ...
          <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
            {user ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <span style={{ color: 'white', fontSize: 14 }}>👋 {user.email}</span>
                <button onClick={fetchMyAppointments}
  style={{ background: 'rgba(255,255,255,0.15)', border: 'none', color: 'white', padding: '8px 16px', borderRadius: 8, cursor: 'pointer', fontWeight: 600, fontFamily: 'inherit' }}>
  📋 Mes RDV
</button>
              ...
              <>
                <button onClick={() => setScreen('login')}
                  style={{ background: 'rgba(255,255,255,0.15)', border: 'none', color: 'white', padding: '8px 18px', borderRadius: 8, cursor: 'pointer', fontWeight: 600, fontFamily: 'inherit' }}>
                  Connexion
                </button>
                <button onClick={() => setScreen('register')}
                  style={{ background: 'white', border: 'none', color: '#0057b8', padding: '8px 18px', borderRadius: 8, cursor: 'pointer', fontWeight: 700, fontFamily: 'inherit' }}>
                  S'inscrire
                </button>
              </>
            )}
      ...
      {/* HERO */}
      <div style={{ background: 'linear-gradient(135deg, #0057b8, #0096c7)', padding: '60px 24px 80px' }}>
        <div style={{ maxWidth: 700, margin: '0 auto', textAlign: 'center' }}>
          <h1 style={{ color: 'white', fontSize: 42, fontWeight: 900, marginBottom: 16, lineHeight: 1.2 }}>
          ...
          <div style={{ background: 'white', borderRadius: 16, padding: 24, boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: 12 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-end' }}>
                <button onClick={() => searchDoctors()}
                  style={{ background: 'linear-gradient(135deg, #0057b8, #0096c7)', color: 'white', border: 'none', padding: '12px 28px', borderRadius: 10, fontSize: 15, fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap', fontFamily: 'inherit' }}>
                  🔍 Rechercher
                </button>
              </div>
            </div>
      ...
      <div style={{ maxWidth: 1100, margin: '-30px auto 0', padding: '0 24px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
          {[['1,240+', 'Médecins partenaires', '👨‍⚕️'], ['58', 'Wilayas couvertes', '📍'], ['24/7', 'Disponible', '⏰']].map(([val, lab, icon]) => (
            <div key={lab} style={{ background: 'white', borderRadius: 14, padding: '20px 24px', textAlign: 'center', boxShadow: '0 4px 20px rgba(0,0,0,0.06)' }}>
              <div style={{ fontSize: 28, marginBottom: 6 }}>{icon}</div>
              <div style={{ fontSize: 28, fontWeight: 800, color: '#0057b8' }}>{val}</div>
              <div style={{ fontSize: 13, color: '#888', marginTop: 4 }}>{lab}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* CONTENT */}
      <div style={{ maxWidth: 1100, margin: '40px auto', padding: '0 24px 60px' }}>
        {showMyRdv && (
  <div style={{ maxWidth: 800, margin: '40px auto', padding: '0 24px 60px' }}>
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
      <h2 style={{ fontWeight: 800, fontSize: 24, color: '#1a1a2e' }}>📋 Mes Rendez-vous</h2>
    ...
    {rdvLoading && (
      <div style={{ textAlign: 'center', padding: 60 }}>
        ...
        <p style={{ color: '#666', marginTop: 16 }}>Chargement...</p>
      </div>
    )}
    ...
    {!rdvLoading && myAppointments.length === 0 && (
      <div style={{ textAlign: 'center', padding: 60, background: 'white', borderRadius: 16 }}>
        <div style={{ fontSize: 48 }}>📭</div>
        <p style={{ color: '#666', marginTop: 16, fontSize: 16 }}>Aucun rendez-vous pour l'instant.</p>
        <button onClick={() => setShowMyRdv(false)}
          style={{ ...S.btnPrimary, marginTop: 20, width: 'auto', padding: '10px 24px' }}>
          Prendre un RDV
        </button>
      </div>
    )}
    ...
    {!rdvLoading && myAppointments.map(appt => (
      <div key={appt.id} style={{ ...S.card, marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: 16, color: '#1a1a2e' }}>
              {appt.doctors?.full_name || 'Médecin inconnu'}
            </div>
            <div style={{ color: '#0057b8', fontSize: 13, fontWeight: 600 }}>
              {appt.doctors?.specialty} · 📍 {appt.doctors?.wilaya}
            </div>
        ...
        <div style={{ textAlign: 'right' }}>
          {/* Status badge */}
          <div style={{ display: 'inline-block', padding: '4px 12px', borderRadius: 99, fontSize: 12, fontWeight: 700, marginBottom: 8,
          ...
          <div style={{ fontWeight: 800, color: '#0057b8', fontSize: 15 }}>
        ...
        {!searched && (
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
              {[['❤️', 'Cardiologue'], ['🦷', 'Dentiste'], ['👁️', 'Ophtalmologue'], ['👶', 'Pédiatre'],
                ['🧴', 'Dermatologue'], ['🤱', 'Gynécologue'], ['🧠', 'Neurologue'], ['🦴', 'Orthopédiste']
              ].map(([icon, name]) => (
                <div key={name}
                  onClick={() => { setSpecialty(name); searchDoctors(name); }}
        ...
        {searched && !loading && (
          <>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <h2 style={{ fontWeight: 800, fontSize: 22, color: '#1a1a2e' }}>
                {doctors.length} médecin(s) trouvé(s)
              </h2>
              <button onClick={() => { setSearched(false); setDoctors([]); setSpecialty('Tous'); setWilaya('Toutes'); }} style={S.btnOutline}>
                ← Retour
              </button>
            </div>
            ...
            {doctors.length === 0 ? (
              <div style={{ textAlign: 'center', padding: 60, background: 'white', borderRadius: 16 }}>
                <div style={{ fontSize: 48 }}>🔍</div>
                <p style={{ color: '#666', marginTop: 16 }}>Aucun médecin trouvé pour cette recherche.</p>
                <p style={{ color: '#aaa', fontSize: 13, marginTop: 8 }}>Essayez une autre spécialité ou wilaya.</p>
              </div>
            ) : (
                ...
                {doctors.map(doc => (
                  <div key={doc.id} style={{ ...S.card, transition: 'transform 0.2s', cursor: 'pointer' }}
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 14, borderTop: '1px solid #f0f4f8' }}>
                      ...
                      <button
                         onClick={() => !user ? setScreen('login') : setBookingDoctor(doc)}
